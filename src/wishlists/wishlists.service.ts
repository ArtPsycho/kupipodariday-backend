...
export class WishlistsService {
  ...
  async create(
    ...
    if (!myWishes || myWishes.length === 0) {
      throw new ForbiddenException(
        'Список желанных подарков должен содержать хотя бы одно для создания',
      );
    }
  ...
  async getWishlistById(id: number): Promise<Wishlist> {
    ...
    if (!wishlist) {
      throw new NotFoundException('Запрошенный список не найден');
    }
    return wishlist;
  }
  async editWishlist(
    ...
    if (wishlist.owner.id !== userId) {
      throw new ForbiddenException(
        'Список создан другим пользователем и не может быть отредактирован',
      );
    }
  ...
  async deleteWishlist(wishlistId: number, userId: number): Promise<Wishlist> {
    ...
    if (wishlist.owner.id !== userId) {
      throw new ForbiddenException(
        'Список создан другим пользователем и не может быть удалён',
      );
    }
